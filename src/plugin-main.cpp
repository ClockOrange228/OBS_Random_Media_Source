...
static bool has_media_ext(const std::string &path)
{
	...
	return std::find(MEDIA_EXTS.begin(), MEDIA_EXTS.end(), ext) !=
	       MEDIA_EXTS.end();
}
...
static void update_file_list(random_media_data *data)
{
	...
	os_dir_t *dir = os_opendir(data->folder.c_str());
	if (!dir) {
		blog(LOG_WARNING, "[RandomMedia] Cannot open folder: %s",
		     data->folder.c_str());
		return;
	}
	struct os_dirent *ent;
	...
	os_closedir(dir);
	blog(LOG_INFO, "[RandomMedia] %zu files in '%s'",
	     data->file_list.size(), data->folder.c_str());
}
		      ...
		      const std::string &file, std::mt19937 &gen)
{
	...
	obs_source_t *media =
		obs_source_create("ffmpeg_source", name.c_str(), s, nullptr);
	obs_data_release(s);

	if (!media) {
		blog(LOG_ERROR, "[RandomMedia] Failed to create source: %s",
		     file.c_str());
		return;
	}
	...
	obs_sceneitem_t *item = obs_scene_add(scene, media);
	if (!item) {
		blog(LOG_ERROR, "[RandomMedia] obs_scene_add failed: %s",
		     file.c_str());
		obs_source_release(media);
		return;
	}
...
static void source_update(void *d, obs_data_t *settings)
{
	...
	data->min_scale =
		static_cast<float>(obs_data_get_double(settings, "min_scale"));
	data->max_scale =
		static_cast<float>(obs_data_get_double(settings, "max_scale"));
	data->preserve_aspect =
		obs_data_get_bool(settings, "preserve_aspect");
	data->min_rot =
		static_cast<float>(obs_data_get_double(settings, "min_rot"));
	data->max_rot =
		static_cast<float>(obs_data_get_double(settings, "max_rot"));
	...
	data->spawn_count =
		static_cast<int>(obs_data_get_int(settings, "spawn_count"));
	data->max_active =
...
static obs_properties_t *source_properties(void *)
{
	...
	obs_properties_add_bool(props, "random_transform",
				"Enable Random Transform");
	obs_properties_add_float(props, "min_scale", "Min Scale (%)", 10.0,
				 1000.0, 1.0);
	obs_properties_add_float(props, "max_scale", "Max Scale (%)", 10.0,
				 1000.0, 1.0);
	obs_properties_add_bool(props, "preserve_aspect",
				"Preserve Aspect Ratio");
	...
	obs_properties_add_bool(props, "hide_on_end",
				"Remove Item After Playback Ends");
	obs_properties_add_int(props, "spawn_count", "Videos per Trigger", 1,
			       10, 1);
